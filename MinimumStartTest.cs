...
namespace BackendEngineer
{
    ...
    public class MinimumStartTest
    {
        ...
        private static int MinimumStart(int[] input)
        {
            ...
            {
                throw new InvalidOperationException("Input values must be greater than 1 and less than 1000000.");
            }

            for (int option = -100; option < 100; option++)
            {
                var value = CalcOption(input, option);
        ...
        private static int CalcOption(int[] input, int initialValue)
        {
